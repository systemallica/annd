...
for i = 1:length(algorithms)
    for j = 1:length(neurons)
        % Create the ANN
        net = feedforwardnet([neurons(j) neurons(j)], algorithms(i));
...
interpolant = scatteredInterpolant(xTest, yTest, tTest');
[X,Y] = meshgrid(linspace(0,1,1000), linspace(0,1,1000));
%mesh(X,Y,interpolant(X,Y));

interpolant1 = scatteredInterpolant(xTest, yTest, a2');
[X1,Y1] = meshgrid(linspace(0,1,1000), linspace(0,1,1000));
%mesh(X1,Y1,interpolant1(X1,Y1));
